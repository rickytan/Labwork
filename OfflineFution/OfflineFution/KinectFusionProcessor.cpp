...
/// </summary>
/// <returns>S_OK on success, otherwise failure code</returns>
HRESULT KinectFusionProcessor::SetWindow(HWND hWnd, UINT msgFrameReady, UINT msgUpdateSensorStatus, UINT msgUpdateNearMode)
{
    AssertOtherThread();
    ...
    m_msgUpdateNearMode = msgUpdateNearMode;
    return S_OK;
}
...
void KinectFusionProcessor::NotifyNearMode()
{
    AssertOwnThread();

    if (m_hWnd != nullptr && m_msgUpdateNearMode != WM_NULL)
    {
        PostMessage(m_hWnd, m_msgUpdateNearMode, m_bNearModeOn, m_bNearModeAvalible);
    }
}
