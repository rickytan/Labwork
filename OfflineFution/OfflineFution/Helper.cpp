...
PCloud Helper::depthFloatToPointCloud(::NUI_FUSION_IMAGE_FRAME * pDepthFloatImage)
{
    ...
    if (LockedRect.Pitch) {
        ...
        //for (unsigned int y = 0; y < pDepthFloatImage->height; ++y)
        {
            ...
            for (unsigned int x = 0; x < pDepthFloatImage->width; ++x)
            {
                ...
                float vz = depth;
                outCloud.points.push_back(pcl::PointXYZ(vx, -vy, -vz));
            }
        }
        );
    }
...
void Helper::savePointCloudTo(const std::string &path, PCloud &cloud, const Eigen::Matrix4f &transform)
{
    ...
    pcl::io::savePLYFile<pcl::PointXYZ>(path, cloud, true);
}
